  ...
  >
    <template #header>
      <div style="display: flex; align-items: center; padding: 10px 0">
        ...
        <span style="font-weight: bold; font-size: 16px">设置氨解状态</span>
      </div>
    </template>
    ...
    <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
      ...
      <el-row :gutter="20">
        <el-col :span="24">
          <el-form-item label="氨解状态" prop="status">
            <el-select v-model="form.status" placeholder="请选择氨解状态" clearable style="width: 100%">
...
const rules = reactive({
  status: [{ required: true, message: '请选择氨解状态', trigger: 'change' }],
  returnReason: [
    {
...
function handleSubmit() {
  proxy.$refs.formRef.validate(valid => {
    if (valid) {
      const msg =
        form.status === 'success'
          ? '成功的项将继续流转到纯化环节，模拟提交成功！'
          : '重合的项将退回反应生产重新安排，模拟提交成功！'
