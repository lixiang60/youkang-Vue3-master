<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button type="warning" plain :icon="Delete" size="small" :disabled="multiple" @click="handleClearWellNo"
          >清除孔号</el-button
        >
      </el-col>
      <el-col :span="1.5">
    ...
    <!-- 4. 业务操作对话框 -->
    <ammonolysis-status-dialog v-model="showStatusDialog" :selected-ids="ids" @success="getList" />
  </div>
</template>

<script setup name="SynthesisAmmonolysis">
import { ref, reactive, onMounted, watch, getCurrentInstance } from 'vue'
import { Edit, Search, Refresh, Delete } from '@element-plus/icons-vue'
import DynamicTable from '@/components/DynamicTable/index.vue'
import DynamicSearch from '@/components/DynamicSearch/index.vue'
import AmmonolysisStatusDialog from './components/AmmonolysisStatusDialog.vue'
...
const STORAGE_KEY = 'synthesis_ammonolysis_columns'
...
function getList() {
  ...
  setTimeout(() => {
    ...
    dataList.value = [
      {
        produceId: '6365320',
        orderNo: '20260417235116087',
        urgent: '否',
        researchGroup: '沈亚欢',
        modification: '-1',
        purification: 'PAGE',
        plateNo: 'cd20260417-4b',
        wellNo: 'C11',
        remark: '测序'
      },
      {
...
function handleClearWellNo() {
  proxy.$modal
    .confirm('确认要清除所选记录的孔号信息吗？该操作不可撤销。')
    .then(() => {
      // TODO 接入清空接口
      proxy.$modal.msgSuccess('模拟清除孔号成功')
      getList()
    })
    .catch(() => {})
}
